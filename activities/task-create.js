  ...
  try {
    ...
    switch (activity.Request.Path) {
      ...
      case "submit":
        ...
        var response = await api.post(`/folders/${personalFolderId}/tasks`, {
          json: true,
          body: {
            title: form.subject,
...
//** filters out "Personal" folder from folders, "Personal" folder cant be deleted from Wrike*/
function getPersonalFolderId(response) {
  let folders = response.body.data;
  for (let i = 0; i < folders.length; i++) {
    if (folders[i].title == "Personal") {
      return folders[i].id;
    }
  }
}
